...
export default function BooksPage() {
    ...
    useEffect(() => {
        async function fetchBooks() {
            try {
                ...
                if (filterAge !== 'all') {
                    // This is a simple filter, might need more complex logic for ranges overlap
                    // For now, let's assume age_range string matching or client side filtering for better UX
                }
    ...
    // Clientside filtering for age ranges to handle overlaps simpler for now
    const filteredBooks = filterAge === 'all'
        ? books
        : books.filter(book => book.age_range.includes(filterAge.split('-')[0])); // Simple check, refine later
                                ...
                                <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 h-full flex flex-col">
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        ...
                                        <div className="flex justify-between items-center mt-auto">
                                            <span className="font-bold text-lg text-gray-900">3.000 F</span>

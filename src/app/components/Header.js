  ...
  useEffect(() => {
    ...
    const updateBadgeCounts = () => {
      ...
      try {
        ...
        // Total (Avoid double counting if sync happens, but for now simple sum or max)
        // Usually items move from guest -> cart.
    ...
    useEffect(() => {
      ...
      window.addEventListener('storage', updateBadgeCounts); // Cross-tab support
